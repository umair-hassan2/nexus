...
(function($) {
   ...
   var clSmoothScroll = function() {
    $('.smoothscroll').on('click', function (e) {
        ...
        // Extract the pathname and hash from the target URL
        var targetPath = this.pathname;
        var currentPath = window.location.pathname;

        // Check if the target is on a different page
        if (targetPath !== currentPath) {
            // Navigate to the target page with hash
            window.location.href = targetPath + target;
        } else {
            // Apply smooth scroll on the current page
            $('html, body').stop().animate({
                ...
                // Update the window's hash to the target
